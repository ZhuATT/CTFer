...
**我们的问题**：指令行（prompt 第 3 段）现在长这样：`[指令] 阶段=exploit；出口判据=confirmed≥1；第 2 轮；已收集（endpoint:32）`——**全是正向数字**。正向数字给 worker 的暗示是"收集了很多，差不多了"。未测面列表虽渲染在状态区，但只是列表没有数字对照，worker 扫一眼不觉得是欠账。A4 文档原文记录过："未测面正确传递但 worker 轮 2 没去测→覆盖引导待优化"；P4.9 依然（V1/V2 idor 三轮未碰）。

**他们的解法**：ARTEX 0.3.5 CHANGELOG 记录真 bug：「目标要求覆盖度 100%、实测仅 40% 却被判完成」。修复 = 把**实测值**（coverage.pct）注入 planner prompt，达标判定前强制核对，"不得以『主要部分已完成/大体达成』为由提前标 met"。教训：没有量化对照，LLM 会拿正向进度自我说服。

**适配方案**：我们不用做他们那么重（他们是 planner 自主判定架构，我们是 driver 机械判定）。只需把欠账显性化——`src/prompt.py::render_round_prompt` 在 directive 段后拼一行：

```
未测面 7 个（地图上有路没探过——目标：清零；上轮 9 个）
```

数字从 `board.untested_surface(tested)` 现算（函数已有，tested 集合 driver 已传）。N=0 时显示「未测面已清零 ✓」。exploit 手册"怎么选方向"段呼应一句："指令行的未测面数字是本轮首要 KPI"。

**成本与边界**：~5 行 + 测试。边界：tested 集合按 endpoint 字符串匹配收集，粒度有限（同端点不同参数算已测），数字可能偏乐观——但"有欠账数字"远好于"只有正向数字"。
