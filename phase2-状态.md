...
- 事实按 kind 分组，每组**上限 12 行**，超出显示"…余 N 条"（防 prompt 膨胀——M2 就定截断，不留到 M4）
- 免疫段独立小节，附"勿重测"
...
- `Blackboard(path)`：load/save JSON（每次 observe 批量后 save，防崩丢）
- `observe(tool_event, round_)`：四类正则抽取入库（去重键：endpoint=method+path / credential=值哈希 / kv_secret=key 名 / fingerprint=名）
...
- `render()`：§1.3 格式 + 免疫段；`plan_directive()` 一行（阶段+出口+轮次）
- **验收（单测）**：10 条假 tool_result 去重入库；喂到 15 条 endpoint 断言 stage→identity；带 cookie 的 provenance 走 verify_fact 断言不重放且 conf 不降；同黑板两次 render 逐字节相同；render 输出被 untrusted 包裹（含恶意 `</untrusted_data` 的 fact 值被消毒）
...
kill 组（同流程变体）：round 1 spawn 后 8s 强杀进程
...
| 风险 | 应对 |
|---|---|
| hxbai 正则是 CTF 题解语境（flag/端口扫），Web 事实抽取可能要大改 | P2.0 先读后定：能抄的只有通用形态（URL/key 形状），业务语义全部重写；以单测假数据为准 |
| Graph State 随事实增长撑爆 prompt | §1.3 定死每组 12 行上限 + 汇总行——第一天就截断，不留隐患 |
| live kill 演示的时序 flaky（杀早了什么都没干） | 固定 8s 延迟杀（selftest round 1 在 ~5s 内必有 init+首批事件）；断言只验"合成 Handoff 存在且 round2 接力成功"，不验合成文本质量 |
| worker 不按指示输出 `<Handoff>` / 不写 FACTS | prompt1 里给逐字模板（含示例行）；relay 断言失败时打印 prompt1/2 全文辅助诊断 |
| 双会话成本 ~$1/次 demo | demo 默认跑，单元层覆盖逻辑；重跑自由 |
| Windows 文件句柄（board 频繁 save 与 runner 写 transcript 同目录） | 不同文件无冲突；board save 用写临时文件+rename 原子替换 |
