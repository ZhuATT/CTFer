...
| **scaffold.py** (127) | `_render_worker_claude:50/expand:72`;WORKER-CLAUDE.md 渲染 | **扩展**:新槽{target}/{hint}/{tools_root}/{env_bg};FORMATS.md/OBSERVER-MANUAL.md;reports\ 与 .observer\ 目录;私有区路径对齐 |
| **guard.py** (135) | 控制器区录像(P-5 修复后 Windows 双形态) | **保留原样** |
...
| **__main__.py** (424) | relay/selftest/run/watch CLI | **微调**:watch 渲染适配新事件 |
...
| # | 任务 | 文件 | 验收 | 决策 |
|---|---|---|---|---|
| T0.1 | 加"身份诊断三实验"节:cookie 摘除/身份注入对调/签名强制性+三场判例(携程/千问/听悟) | `.claude/skills/auth-token/SKILL.md` | 节+判例在;description 不动 | A12 迁移/N5 |
| T0.2 | 加 IDOR 必做闭环(双账号/对象枚举/身份对调/响应 diff/写读回)+攻击四步 | `.claude/skills/auth-access/SKILL.md` | 五步+四步在 | A12/N5 |
| T0.3 | 补:浏览器四步侦察/资产分诊/**WaitForMcpServers 第 0 步(P-7 实测教训,随手册①迁移不得丢)**/收尾画像行(冷启动措辞) | `.claude/skills/recon-methodology/SKILL.md` | 四件全在;画像行含前缀教学 | A12/A14/P-7 |
| T0.4 | 产出规范加 description 触发式守门条款 | `.claude/skills/distill-report/SKILL.md` | "Load when 收尾"一句在 | A22 |

### P1 图核心(board.py,6 件)

| # | 任务 | 锚点 | 验收 | 决策 |
|---|---|---|---|---|
| T1.1 | blackboard.json v3 容器(graph/bookkeeping)+**v2→v3 迁移脚本**(旧黑板 facts/findings/directions/immune/chains/session_intel → nodes/edges/播种) | `_load:200/save:235` 重写 | 旧 usc/usc-fresh 黑板迁移跑通;免疫→阴性视图公式核对 | §三/S 系列 |
| T1.2 | 节点模型:三 kind payload/状态机/公共 7 字段;三原子操作(controller 独占);六不变量;去重键;id 重编号 | 替换 add_fact/add_finding/add_direction/merge_directions | schema §2/§3 契约测试全过 | schema |
| T1.3 | 边模型:六动词+方向约定+origin+兜底(声明优先,endpoint 兜底) | 替换 add_chains | 兜底单测 | schema §4 |
| T1.4 | 四派生视图(零存储现算):阴性/端点分组/谱系/未测面 | 新函数 | 四视图单测(usc 数据回放) | §三 |
| T1.5 | goal 字段(text/updated_round,控制台可改)+check_goal 重写:删 stage 机,TERMINAL=预算耗尽/人工停/**有效 Stop**(达成须引 F-xxx) | `check_goal:568/_advance:604` 重写 | Stop 解析单测(无引证=无效);O-4 随灭 | S5/S6/A18 |
| T1.6 | 删除执行:被动抽取 `_extract_facts:124`/`observe/ingest_facts` 抽取路径/事实衰减 `verify_facts_against_transcript:610`/immune 存储 `add_immune:343`/`update_session_intel:373`/旧渲染全家(render:745/render_summary:834/render_yaml_layer:857/plan_directive:922/intel_summary:917) | 见左 | 术语清扫零残留(T6.1 联动) | 报废清单/A15/A16 |

### P2 收割投影写回(7 件)

| # | 任务 | 锚点 | 验收 | 决策 |
|---|---|---|---|---|
| T2.1 | 配方0 播种:prior-intel/status.md 漏洞表→节点(confirmed,origin=user);`parse_immune_from_status` 改播种 | writeback:197 | usc-fresh 播种回放 | schema §6 |
| T2.2 | 配方1 轮末收割:FINDINGS(报告指针+chain 声明边+endpoint 兜底 yields)/FACTS(evidence 透传)/**HANDOFF→bookkeeping(读者=观察者)/STOP 抽取→TERMINAL**;DIRECTIONS 行废 | `_harvest_findings:52` 改;`_harvest_directions:193/_handoff_unfinished_to_directions:215` 删 | usc 增量账本收割回放 | S7/A19 |
...
| T2.5 | writeback:三件保留;immune→阴性视图化;报告晋升 .auto\reports→reports\;ledger_synced 保留 | writeback.py 改造 | usc 收尾三件+晋升 e2e | §七 |
...
| # | 任务 | 文件 | 验收 | 决策 |
|---|---|---|---|---|
| T3.1 | WORKER-CLAUDE.md 六节:身份开工({target}/{hint},铁律,Read STATE.md 第一件事,图空冷启动,**{env_bg} 长任务纪律**)/判层契约(A2 三层+画像指引)/写盘义务(FORMATS 必读+log.jsonl 记账[P-6 纪律保留])/干活规则(探透判定+自由条款+探索义务三出口+**<Stop> 信号格式**)/台账纪律/写操作约束;**{tools_root} 环境槽**(js-intel 路径) | `scaffolding/WORKER-CLAUDE.md` | 渲染产物无 v2 残留词;六节齐 | A1-A6/A15/A16/A18/A21 |
...
| T3.3 | scaffold 扩展:双手册+FORMATS 每轮重写(防篡改)/reports\ 与 .observer\ 目录/私有区路径对齐 v3/新槽渲染 | `src/scaffold.py` | dry-run 渲染产物检查 | A8/A17 |
...
| T5.2 | observer.py 重写:chat 双通道退役;-p 同构 spawn(复用 runner 通道,全能力,`.observer\` 工作台);OBSERVER 七类行输出;0-finding 轮照跑(P-3 语义保留);**角色模型配置化**(A23:双角色 model/baseurl/ak 可配,默认 worker=glm-5.3-flash/观察者=glm-5.3 同 bigmodel;配置文件→控制台) | observer.py 重写 | 七类行解析单测;双角色配置生效单测;真通道冒烟 | A17/A23/S7/P-13 |
...
| T6.2 | tests 全量重写:节点/边/三原子/六不变量/四视图/配方0-2/七类行/Stop/槽渲染/路由三信号/迁移脚本;保留 guard/noreport/stoploss/preflight/cli 既有测试 | 全绿(基线 219 起,只增不减) |
...
| 风险 | 对策 |
|---|---|
| board.py 961 行核心重写,量最大 | schema 契约即规格(T1.2/T1.3 逐条对 schema);usc/usc-fresh 真实黑板作迁移回放夹具;每阶段全绿红线 |
...
**开工顺序**:T0(P0)与 T1.1-T1.2(P1)并行起步;首件交付=图容器+迁移脚本(一切的地基)。
