"""AT1 llm —— 门 2 verifier 的 OpenAI 兼容客户端（vendor 自 hxbai llm.py）。

改动：config 引用改 providers；去掉 observability 集成（我们用 events，由调用方记）。
保留：限流重试 + 指数退避、空响应重试、fallback 后备链、reasoning_effort 兼容退避。
"""
